...
class SystemHealthService:
    ...
    def get_system_health(rbd_colegio=None):
        ...
        # 1. Obtener estado de setup (si se especifica colegio)
        setup_status = None
        if rbd_colegio:
            setup_status = SetupService.get_setup_status(rbd_colegio)
        
        # 2. Auditar integridad de datos
        data_integrity = SystemHealthService._audit_data_integrity(rbd_colegio)
        
        # 3. Identificar problemas críticos y warnings
        critical_issues = []
        warnings = []
        ...
        # 4. Determinar salud general del sistema
