...
def estadisticas_becas(request):
    ...
    try:
        ...
        anio_actual = ORMAccessService.filter(
            Matricula,
            colegio_id=rbd,
            estado="ACTIVA"
        ).values_list("anio_escolar", flat=True).order_by("-anio_escolar").first()
        ...
        becas = ORMAccessService.filter(
            ...
            matricula__anio_escolar=anio_actual
        )
        ...
        if becas_por_matricula:
            ...
            for cuota in cuotas:
                porcentaje = becas_por_matricula.get(cuota['matricula_id']) or 0
                monto_total += cuota['monto_original'] * (porcentaje / 100)
...
def listar_becas(request):
    ...
    try:
        ...
        becas = ORMAccessService.filter(Beca, matricula__colegio_id=rbd).select_related('estudiante', 'aprobada_por')
