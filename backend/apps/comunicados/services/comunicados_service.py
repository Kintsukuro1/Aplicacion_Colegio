...
class ComunicadosService:
    ...
    def notify_new_comunicado(comunicado) -> None:
        ...
        for usuario in destinatarios:
            ...
            if comunicado.es_prioritario and usuario.email:
                send_mail(
                    subject=f"{icono} {comunicado.titulo}",
                    message=(
                        f"{comunicado.contenido}\n\n---\n"
                        f"Publicado por: {comunicado.publicado_por.get_full_name()}\n"
                        f"{comunicado.colegio.nombre}"
                    ),
