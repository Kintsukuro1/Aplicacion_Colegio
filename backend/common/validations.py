...
class CommonValidations:
    ...
    def validate_beca_data(data: dict, operation_type: str = 'create') -> Tuple[bool, Optional[str]]:
        ...
        # Validar que el estudiante existe y pertenece al colegio
        if 'estudiante_id' in data:
            from apps.core.models import Estudiante
            try:
                estudiante = Estudiante.objects.get(id=data['estudiante_id'])
                # Validar que el estudiante esté matriculado
                if not estudiante.matriculas.filter(activa=True).exists():
                    errors.append("El estudiante debe tener una matrícula activa")
    ...
    def validate_boleta_data(data: dict, operation_type: str = 'create') -> Tuple[bool, Optional[str]]:
        ...
        if 'cuota_id' in data:
            from apps.core.models import Cuota
            try:
                cuota = Cuota.objects.get(id=data['cuota_id'])
                # Validar que la cuota esté activa
                if not cuota.activa:
                    errors.append("La cuota debe estar activa")
