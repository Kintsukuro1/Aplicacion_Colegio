...
class TestDashboardContextServiceFallbacks:
    ...
    def test_get_profesor_perfil_context_handles_internal_exception(self):
        user = Mock(id=7)
        eval_distinct = Mock()
        eval_distinct.values.return_value.distinct.return_value.count.return_value = 3
        eval_count = Mock()
        ...
        with patch('backend.apps.academico.models.Evaluacion.objects.filter', side_effect=[eval_distinct, eval_count]), patch(
